...
class readUserdataState(EventState):
        '''
        Example for a state which will read some input userdata (int) and log/print it without making changes
        ...
        def __init__(self, text, severity=Logger.REPORT_HINT):
                ...
                super(readUserdataState, self).__init__(outcomes = ['continue', 'failed'],
        ...
        def execute(self, userdata):
                ...
                try:
                  Logger.log(self._text.format(userdata.input_userdata), self._severity)
                  return 'continue'
                except:
                  return 'failed'
